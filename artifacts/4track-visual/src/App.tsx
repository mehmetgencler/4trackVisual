...
import { DiagnosticAuditPanel } from './components/DiagnosticAuditPanel';
...
export default function App() {
  ...
  const handleLoadSyntheticStems = (demoTracks: TrackData[]) => {
    schedulerRef.current?.stop();
    alignTrackLengths(demoTracks);
    setTracks(demoTracks);
    schedulerRef.current?.setTracks(demoTracks);
    triggerEngineRef.current.reset();
    setActiveRings([]);
    setActiveParticles([]);
    setIsDiagnosticsOpen(false);
    setTimeout(() => schedulerRef.current?.play(0), 100);
  };
  ...
  return (
    <div className="studio-grain min-h-screen bg-[#0A0A0B] text-[#E0E0E0] flex flex-col font-sans selection:bg-cyan-500 selection:text-black">
        ...
        <div className="flex items-center space-x-4 sm:space-x-6 text-[10px] font-mono text-neutral-400">
          ...
          >
           ...
           <button
             ...
             className={`px-3 py-1 text-[10px] font-mono border uppercase transition-colors flex items-center gap-1.5 ${
               isDiagnosticsOpen
                 ? 'border-amber-400/60 bg-amber-400/10 text-amber-300'
                 : 'border-[#333] bg-[#1a1a1c] text-neutral-400 hover:text-neutral-100 hover:bg-[#252528]'
             }`}
      ...
      <DiagnosticAuditPanel
        ...
        onToggle={() => setIsDiagnosticsOpen((open) => !open)}
        onLoadSyntheticStems={handleLoadSyntheticStems}
      />
