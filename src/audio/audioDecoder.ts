...
export async function decodeAudioFile(file: File): Promise<AudioBuffer> {
  ...
  try {
    ...
    if (standardBuffer && standardBuffer.length > 0) {
      ...
      logger.success(
        'DECODER',
        `Decoded "${file.name}" via native Web Audio API in ${duration.toFixed(1)}ms (${standardBuffer.numberOfChannels}ch, ${standardBuffer.sampleRate}Hz, ${standardBuffer.duration.toFixed(1)}s)`
      );
 ...
 */
async function parseWavFile(buffer: ArrayBuffer): Promise<AudioBuffer> {
  ...
  // Allocate temporary channel arrays and fill them (avoid createBuffer until we know it will succeed)
  const channelArrays: Float32Array[] = new Array(numChannels);
  for (let c = 0; c < numChannels; c++) channelArrays[c] = new Float32Array(numFrames);

  const viewLen = totalLength;
  ...
  if (audioFormat === 3 || (audioFormat === 0xfffe && bitsPerSample === 32)) {
    ...
    for (let f = 0; f < numFrames; f++) {
      for (let c = 0; c < numChannels; c++) {
        channelArrays[c][f] = readPtr + 4 <= viewLen ? view.getFloat32(readPtr, true) : 0;
        readPtr += 4;
      }
    }
    ...
    for (let f = 0; f < numFrames; f++) {
      for (let c = 0; c < numChannels; c++) {
        channelArrays[c][f] = readPtr + 2 <= viewLen ? view.getInt16(readPtr, true) / 32768 : 0;
        readPtr += 2;
      }
    }
    ...
    for (let f = 0; f < numFrames; f++) {
      for (let c = 0; c < numChannels; c++) {
        if (readPtr + 3 <= viewLen) {
          ...
          channelArrays[c][f] = sample32 / 8388608;
        } else {
          channelArrays[c][f] = 0;
        }
        readPtr += stride;
    ...
    for (let f = 0; f < numFrames; f++) {
      for (let c = 0; c < numChannels; c++) {
        channelArrays[c][f] = readPtr + 4 <= viewLen ? view.getInt32(readPtr, true) / 2147483648 : 0;
        readPtr += 4;
      }
    }
    ...
    for (let f = 0; f < numFrames; f++) {
      for (let c = 0; c < numChannels; c++) {
        channelArrays[c][f] = readPtr + 1 <= viewLen ? (view.getUint8(readPtr) - 128) / 128 : 0;
        readPtr += 1;
      }
    }
  } else {
  ...
  // Use resampler helper to safely create an AudioBuffer compatible with the shared AudioContext
  const audioBuffer = await ensureBufferInContext(sampleRate, channelArrays, numFrames);
  return audioBuffer;
}
 ...
 */
async function parseAiffFile(buffer: ArrayBuffer): Promise<AudioBuffer> {
  const view = new DataView(buffer);
  const totalLength = buffer.byteLength;
  let offset = 12;
  ...
  if (bitsPerSample === 16) {
    for (let f = 0; f < numFrames; f++) {
      for (let c = 0; c < numChannels; c++) {
        channelArrays[c][f] = view.getInt16(readPtr, false) / 32768; // Big-Endian
        readPtr += 2;
      }
    }
  } else if (bitsPerSample === 24) {
    ...
    for (let f = 0; f < numFrames; f++) {
      for (let c = 0; c < numChannels; c++) {
        channelArrays[c][f] = view.getInt32(readPtr, false) / 2147483648;
        readPtr += 4;
      }
    }
  }
